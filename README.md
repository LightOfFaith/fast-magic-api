...
```shell
## 构建Docker镜像
docker build -f fast-magic-api.Dockerfile -t fast-magic-api:1.0.0 .
## 运行Docker容器
docker run -it --rm --name fast-magic-api -p 9090:8080 -d \
  -e JAVA_OPTS="-Xms256m -Xmx256m -XX:MetaspaceSize=256m -XX:MaxMetaspaceSize=256m -Duser.timezone=Asia/Shanghai -Dfile.encoding=UTF-8 -Dspring.profiles.active=test -D=server.port=8080" \
